...
function ensureDefaultSubcats() {
  const defs = {
    коммуналка: { type: "expense", subs: ["свет", "вода", "газ", "сбор мусора", "интернет"] },
    продукты: { type: "expense", subs: ["овощи", "фрукты", "мясо", "рыба", "молочные", "хлеб"] },
    транспорт: { type: "expense", subs: ["метро", "автобус", "такси", "бензин"] }
  };
  for (const [cat, { type, subs }] of Object.entries(defs)) {
    ensureGroup(cat, type);
    for (const s of subs) if (!categoryGroups[cat][type].subcats.includes(s)) categoryGroups[cat][type].subcats.push(s);
  }
}
...
function removeSubcat(cat, type, sub) {
  if (categoryGroups[cat]?.[type]) {
    categoryGroups[cat][type].subcats = categoryGroups[cat][type].subcats.filter(s => s !== sub);
    saveAll();
  }
}
...
function renderCalcPreview() {
  ...
  if (!el) return;
  if (calcHistory.length === 0) { el.textContent = "Нет истории"; return; }
...
function saveNotebook() { localStorage.setItem("notebook_v2", JSON.stringify(notebookPages)); }
function renderNotebookList() {
  const container = $("notebookList");
  if (!container) return;
  container.innerHTML = "";
  if (notebookPages.length === 0) { container.innerHTML = '<div class="empty-msg">Нет страниц. Нажмите ➕ для создания.</div>'; return; }
  const sorted = [...notebookPages].sort((a, b) => (b.date || "").localeCompare(a.date || ""));
  sorted.forEach(page => {
    const card = document.createElement("div");
...
function saveNotebookPage() {
  ...
  const conflict = notebookPages.some(p => p.id !== currentNbId && p.title.toLowerCase() === title.toLowerCase());
  if (conflict) { alert(`Страница "${title}" уже существует`); return; }
  const page = notebookPages.find(p => p.id === currentNbId);
  if (page) { page.title = title; page.date = date; page.content = content; }
  saveNotebook();
  renderNotebookList();
...
function createNotebookPage() {
  let maxNum = 0;
  notebookPages.forEach(p => { const m = p.title.match(/Страница (\d+)/i); if (m && parseInt(m[1]) > maxNum) maxNum = parseInt(m[1]); });
...
function updateBalance() {
  let inc = 0, exp = 0;
  for (const t of transactions) { if (t.type === "income") inc += t.amountRub; else exp += t.amountRub; }
  const s = sym();
...
function buildOpCard(op, deleteIndex) {
  ...
  `;
  card.querySelector(".op-del").addEventListener("click", (e) => { e.stopPropagation(); if (confirm("Удалить операцию?")) { transactions.splice(deleteIndex, 1); saveAll(); refreshAll(); } });
  card.addEventListener("click", (e) => { if (e.target.classList.contains("op-del")) return; openEditOpModal(deleteIndex); });
  return card;
}
function openEditOpModal(index) {
  ...
  catSelect.innerHTML = "";
  cats.forEach(c => catSelect.appendChild(new Option(`${getEmoji(c)} ${c}`, c)));
  ...
  if (op.subcategory && getSubcats(op.category, op.type).includes(op.subcategory)) {
    subcatField.style.display = "flex";
    subcatSelect.value = op.subcategory;
  } else { subcatField.style.display = "none"; }
  openModal("editOpModal");
}
function refreshEditModalSubcats() {
  const cat = $("editModalCat").value;
  const type = $("editTypeIncomeBtn").classList.contains("active") ? "income" : "expense";
  const subs = getSubcats(cat, type);
  const field = $("editModalSubcatField");
  const sel = $("editModalSubcat");
  if (subs.length > 0) { field.style.display = "flex"; sel.innerHTML = '<option value="">— не указывать —</option>'; subs.forEach(s => sel.appendChild(new Option(s, s))); }
...
function saveEditedOp() {
  ...
  const type = $("editTypeIncomeBtn").classList.contains("active") ? "income" : "expense";
  const cat = $("editModalCat").value;
  const subcat = $("editModalSubcatField").style.display !== "none" ? $("editModalSubcat").value : "";
  ...
  transactions[editingOpIndex] = { type, category: cat, subcategory: subcat || null, amountRub, note: note || null, date };
  saveAll(); refreshAll(); closeModal("editOpModal"); editingOpIndex = null;
}
...
function renderRecentOps() {
  ...
  if (sorted.length === 0) { container.innerHTML = '<div class="empty-msg">Нет операций. Нажмите ＋ чтобы добавить.</div>'; return; }
...
function renderAllOps() {
  ...
  if (sorted.length === 0) { container.innerHTML = '<div class="empty-msg">Нет операций по заданным критериям.</div>'; return; }
  for (const op of sorted) container.appendChild(buildOpCard(op, op._i));
}

// ========== CATEGORIES MANAGER ==========
function buildSubcatSection(cat, type, label, subs) {
  const chips = subs.map(s => `<span class="subcat-chip" data-cat="${esc(cat)}" data-type="${type}" data-sub="${esc(s)}">${esc(s)}<button class="subcat-del" data-cat="${esc(cat)}" data-type="${type}" data-sub="${esc(s)}">✕</button></span>`).join("");
...
function renderCatManager() {
  ...
  for (const catName of allCats) {
    ...
    let badge = "";
    if (inInc && inExp) badge = '<span class="cat-type-badge badge-both">🔄 Оба</span>';
    else if (inInc) badge = '<span class="cat-type-badge badge-income">💰 Доход</span>';
    ...
    if (inInc && inExp) {
      ...
      if (incS.length) subcatHtml += buildSubcatSection(catName, "income", "📈 Доход", incS);
      if (expS.length) subcatHtml += buildSubcatSection(catName, "expense", "📉 Расход", expS);
    } else {
      const type = inInc ? "income" : "expense";
      const subs = getSubcats(catName, type);
      if (subs.length) subcatHtml += buildSubcatSection(catName, type, "", subs);
    }
    const card = document.createElement("div");
    ...
    card.querySelector(".cat-name-text").addEventListener("click", () => { const newName = prompt(`Переименовать "${catName}":`, catName); if (newName && newName.trim() && newName.trim() !== catName) renameCategory(catName, newName.trim()); });
    card.querySelectorAll(".add-sub-btn").forEach(btn => btn.addEventListener("click", () => { const cat = btn.dataset.cat, type = btn.dataset.type, sub = prompt(`Подкатегория для "${cat}":`); if (sub?.trim() && !getSubcats(cat, type).includes(sub.trim())) { addSubcat(cat, type, sub.trim()); renderCatManager(); refreshModalCats(); } }));
    card.querySelector(".del-cat-btn").addEventListener("click", () => { if (confirm(`Удалить категорию "${catName}"?`)) { incomeCategories = incomeCategories.filter(c => c !== catName); expenseCategories = expenseCategories.filter(c => c !== catName); delete categoryGroups[catName]; saveAll(); renderCatManager(); refreshModalCats(); updateBalance(); } });
    container.appendChild(card);
  }
  document.querySelectorAll(".subcat-chip").forEach(chip => { chip.addEventListener("click", (e) => { if (e.target.classList.contains("subcat-del")) return; const cat = chip.dataset.cat, type = chip.dataset.type, sub = chip.dataset.sub; openEditSubcatModal(cat, type, sub); }); });
  document.querySelectorAll(".subcat-del").forEach(btn => { btn.addEventListener("click", (e) => { e.stopPropagation(); const cat = btn.dataset.cat, type = btn.dataset.type, sub = btn.dataset.sub; if (confirm(`Удалить подкатегорию "${sub}"?`)) { removeSubcat(cat, type, sub); renderCatManager(); refreshModalCats(); } }); });
}
function renameCategory(oldName, newName) {
  const all = [...incomeCategories, ...expenseCategories];
  if (all.includes(newName)) { alert(`Категория "${newName}" уже существует`); return; }
  incomeCategories = incomeCategories.map(c => c === oldName ? newName : c);
  expenseCategories = expenseCategories.map(c => c === oldName ? newName : c);
  if (categoryGroups[oldName]) { categoryGroups[newName] = categoryGroups[oldName]; delete categoryGroups[oldName]; }
...
function openEditSubcatModal(cat, type, sub) {
  ...
  modal.dataset.cat = cat; modal.dataset.type = type; modal.dataset.oldSub = sub;
  openModal("editSubcatModal");
}
function saveSubcatEdit() {
  const modal = $("editSubcatModal");
  const cat = modal.dataset.cat, type = modal.dataset.type, oldSub = modal.dataset.oldSub;
  const newSub = $("editSubcatName").value.trim();
  if (!newSub) { alert("Название не может быть пустым"); return; }
  if (oldSub !== newSub) {
    const subs = getSubcats(cat, type);
    if (subs.includes(newSub)) { alert("Такая подкатегория уже существует"); return; }
    const idx = subs.indexOf(oldSub);
    if (idx !== -1) { subs[idx] = newSub; categoryGroups[cat][type].subcats = subs; saveAll(); renderCatManager(); refreshModalCats(); }
  }
  closeModal("editSubcatModal");
}
function deleteSubcatFromModal() {
  const modal = $("editSubcatModal");
  const cat = modal.dataset.cat, type = modal.dataset.type, sub = modal.dataset.oldSub;
  if (confirm(`Удалить подкатегорию "${sub}"?`)) { removeSubcat(cat, type, sub); renderCatManager(); refreshModalCats(); closeModal("editSubcatModal"); }
}
...
function setOpType(type) { currentOpType = type; $("typeExpenseBtn").className = "type-btn" + (type === "expense" ? " active" : ""); $("typeIncomeBtn").className = "type-btn" + (type === "income" ? " active" : ""); refreshModalCats(); }
function submitOp() {
  const cat = $("modalCat").value;
  const subcat = $("modalSubcatField").style.display !== "none" ? $("modalSubcat").value : "";
  const amount = parseFloat($("modalAmount").value);
  const note = $("modalNote").value.trim();
  const date = $("modalDate").value || today();
  if (!cat) { alert("Выберите категорию"); return; }
  if (isNaN(amount) || amount <= 0) { alert("Введите сумму больше 0"); return; }
  transactions.push({ type: currentOpType, category: cat, subcategory: subcat || null, amountRub: toRub(amount), note: note || null, date });
  saveAll(); refreshAll(); closeModal("addOpModal"); $("modalAmount").value = ""; $("modalNote").value = "";
}
function openAddOpModal() { $("modalDate").value = today(); $("modalAmount").value = ""; $("modalNote").value = ""; setOpType("expense"); openModal("addOpModal"); setTimeout(() => $("modalAmount").focus(), 120); }
function deleteCurrentModalCat() {
  const val = $("modalCat").value;
  if (!val) return;
  const arr = currentOpType === "income" ? incomeCategories : expenseCategories;
  if (arr.length <= 1) { alert("Нельзя удалить последнюю категорию"); return; }
  if (!confirm(`Удалить категорию "${val}"?`)) return;
  if (currentOpType === "income") incomeCategories = incomeCategories.filter(c => c !== val);
  else expenseCategories = expenseCategories.filter(c => c !== val);
  saveAll(); renderCatManager(); refreshModalCats();
}

// ========== CATEGORY MODAL ==========
function openAddCatModal() { selectedCatType = null; $("addCatName").value = ""; $("confirmAddCat").disabled = true; document.querySelectorAll("#catBtnExpense, #catBtnIncome, #catBtnBoth").forEach(btn => btn.className = "cat-type-btn"); openModal("addCatModal"); setTimeout(() => $("addCatName").focus(), 120); }
window.selectCatType = function(type) {
  selectedCatType = type;
  ["catBtnExpense", "catBtnIncome", "catBtnBoth"].forEach(id => $(id).className = "cat-type-btn");
  if (type === "expense") $("catBtnExpense").className = "cat-type-btn selected-expense";
  if (type === "income") $("catBtnIncome").className = "cat-type-btn selected-income";
...
window.confirmAddCategory = function() {
  ...
  let added = false;
  if (selectedCatType === "expense" || selectedCatType === "both") { if (!expenseCategories.includes(name)) { expenseCategories.push(name); added = true; } }
  if (selectedCatType === "income" || selectedCatType === "both") { if (!incomeCategories.includes(name)) { incomeCategories.push(name); added = true; } }
  if (!added) { alert(`Категория "${name}" уже существует`); return; }
  if (selectedCatType === "both") { ensureGroup(name, "income"); ensureGroup(name, "expense"); } else ensureGroup(name, selectedCatType);
  saveAll(); renderCatManager(); refreshModalCats(); closeModal("addCatModal");
};
...
function calcEval() {
  if (!calcExpr) return;
  try { const res = Function('"use strict"; return (' + calcExpr + ")")(); if (isFinite(res)) { const result = +res.toPrecision(12); addCalcHistory(calcExpr, result); calcExpr = String(result); calcJustEvaled = true; } else { calcExpr = "Ошибка"; } } catch { calcExpr = "Ошибка"; calcJustEvaled = false; }
  renderCalcDisplay();
}
function handleCalc(action) {
  const isOp = "+-*/".includes(action);
  if (calcExpr === "Ошибка") { calcExpr = ""; calcJustEvaled = false; }
  if (action === "clear") { calcExpr = ""; calcJustEvaled = false; }
  else if (action === "back") { if (calcJustEvaled) { calcExpr = ""; calcJustEvaled = false; } else calcExpr = calcExpr.slice(0, -1); }
  else if (action === "=") { calcEval(); return; }
  else if (action === "sign") { if (!calcExpr || calcExpr === "0") { calcExpr = "-"; calcJustEvaled = false; } else if (!isNaN(calcExpr)) { calcExpr = String(-+calcExpr); calcJustEvaled = false; } else { calcExpr = calcExpr.startsWith("-(") ? calcExpr.slice(2, -1) : `-(${calcExpr})`; calcJustEvaled = false; } }
  else if (action === "%") { try { const v = Function('"use strict"; return (' + calcExpr + ")")(); if (isFinite(v)) { calcExpr = String(v / 100); calcJustEvaled = false; } } catch {} }
  else { if (calcJustEvaled) { if (!isOp) calcExpr = ""; calcJustEvaled = false; } if (isOp && calcExpr && "+-*/".includes(calcExpr.slice(-1))) calcExpr = calcExpr.slice(0, -1); if (action === "." && calcExpr.split(/[+\-*/]/).pop().includes(".")) { renderCalcDisplay(); return; } calcExpr += action; }
  renderCalcDisplay();
}
function buildCalcGrid() {
  const keys = [["C","clear"],["⌫","back"],["%","%"],["÷","/"],["7","7"],["8","8"],["9","9"],["×","*"],["4","4"],["5","5"],["6","6"],["−","-"],["1","1"],["2","2"],["3","3"],["+","+"],["+/−","sign"],["0","0"],[".","."],["=","="]];
  const grid = $("calcGrid");
  if (!grid) return;
  grid.innerHTML = "";
  keys.forEach(([label, action]) => { const btn = document.createElement("button"); btn.textContent = label; btn.className = "calc-btn"; if (action === "clear" || action === "back") btn.classList.add("clear"); if (["/","*","-","+","="].includes(action)) btn.classList.add("op"); btn.addEventListener("click", () => handleCalc(action)); grid.appendChild(btn); });
}
function renderCalcHistoryModal() {
  const container = $("calcHistoryList");
  if (!container) return;
  container.innerHTML = "";
  if (calcHistory.length === 0) { container.innerHTML = '<div class="empty-msg">История пуста</div>'; return; }
...
async function fetchRates() {
  ...
  try {
    ...
    saveAll();
  } catch { if (statusEl) statusEl.textContent = lastRateUpdate ? `⚠️ Офлайн. Последнее: ${lastRateUpdate}` : "⚠️ Офлайн (используются встроенные курсы)"; }
  refreshAll();
}
...
function initTheme() {
  ...
  chk.addEventListener("change", (e) => { const dark = e.target.checked; document.body.classList.toggle("dark", dark); localStorage.setItem("budget_theme", dark ? "dark" : "light"); });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("closeCalcHistoryModal").addEventListener("click", () => closeModal("calcHistoryModal"));
  ...
  $("addCatName").addEventListener("keydown", e => { if (e.key === "Enter" && selectedCatType) confirmAddCategory(); });
  $("openCalcHistoryBtn").addEventListener("click", () => { renderCalcHistoryModal(); openModal("calcHistoryModal"); });
  $("clearCalcHistoryBtn").addEventListener("click", () => { if (confirm("Очистить историю?")) { calcHistory = []; saveCalcHistory(); renderCalcHistoryModal(); renderCalcPreview(); } });
  ...
  $("modalRepeatBtn").addEventListener("click", () => { const last = [...transactions].reverse().find(t => t.type === "expense"); if (!last) { alert("Нет расходов"); return; } setOpType("expense"); setTimeout(() => { $("modalCat").value = last.category; refreshModalSubcats(); if (last.subcategory) $("modalSubcat").value = last.subcategory; $("modalAmount").focus(); }, 50); });
  ...
  $("editTypeIncomeBtn").addEventListener("click", () => { $("editTypeIncomeBtn").classList.add("active"); $("editTypeExpenseBtn").classList.remove("active"); refreshEditModalSubcats(); });
