    ...
    <div class="settings-card">
      ...
      <div class="settings-card-body">
                ...
                typeof Notification === "undefined"
                  ...
                  : Notification.permission === "granted"
                    ? {
                        ...
                        ka: "✅ ნებართვა მიღებულია",
                      }[currentLang]
                    : Notification.permission === "denied"
                      ? {
                          ru: "⛔ Заблокировано — откройте настройки браузера",
                          en: "⛔ Blocked — open browser settings",
                          ka: "⛔ დაბლოკილია — ბრაუზერის პარამეტრები",
                        }[currentLang]
                      : {
                          ru: "Нажмите «Разрешить» чтобы получать уведомления",
                          en: "Tap 'Allow' to receive notifications",
                          ka: "'ნება' დაჭირეთ",
                        }[currentLang]
              }</div>
            </div>
        ...
        <div style="display:flex;gap:8px;margin-bottom:20px;">
          <button id="notifEnableBtn" class="btn-primary" style="flex:1;padding:12px;">
            🔔 ${
              remindersEnabled
                ? {
                    ru: "Выключить напоминания",
          ...
          ${
            typeof Notification !== "undefined" &&
            Notification.permission === "denied"
              ? `<button id="notifHelpBtn" class="btn-secondary" style="padding:12px;font-size:13px;">❓</button>`
              : ""
          }
        </div>

        <!-- Named reminders list -->
        <div id="namedRemindersList">
          ${(() => {
            ...
            if (!list.length)
              return `<div style="font-size:13px;color:var(--text-muted);padding:8px 0 16px;">
              ${{ ru: "Нет запланированных напоминаний", en: "No scheduled reminders", ka: "შეხსენებები არ არის" }[currentLang]}
            </div>`;
            return list
              .map(
                (r, i) => `
              <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:${
                r.fired ? "var(--cream-dark)" : "var(--income-pale)"
              };border:1.5px solid ${r.fired ? "var(--cream-border)" : "var(--income-color)"};border-radius:12px;margin-bottom:8px;">
                <div style="flex:1;min-width:0;">
                  <div style="font-weight:700;font-size:13px;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(r.name || "🔔")}</div>
                  <div style="font-size:11px;color:var(--text-muted);">${new Date(r.ts).toLocaleString(currentLang === "en" ? "en-US" : currentLang === "ka" ? "ka-GE" : "ru-RU")}</div>
                </div>
                <span style="font-size:12px;color:${r.fired ? "var(--text-muted)" : "var(--income-color)"};font-weight:700;">${
                  r.fired
                    ? { ru: "✓ Выполнено", en: "✓ Done", ka: "✓ შესრულდა" }[
                        currentLang
                      ]
                    : { ru: "⏰ Ждёт", en: "⏰ Pending", ka: "⏰ ელოდება" }[
                        currentLang
                      ]
                }</span>
...
<div style="background:var(--cream-dark);border-radius:14px;padding:14px;border:1.5px solid var(--cream-border);">
      ...
      🔔 ${{ ru: "Тест", en: "Test", ka: "ტესტი" }[currentLang]}
        ...
        <div style="margin-top:20px;">
          <div style="font-size:14px;font-weight:800;color:var(--text);margin-bottom:10px;">
            🔁 ${{ ru: "Повторяющиеся напоминания", en: "Recurring reminders", ka: "განმეორებადი შეხსენებები" }[currentLang]}
          </div>
          ...
          ${[
            ...
            {
              val: "daily",
              label: { ru: "Каждый день", en: "Daily", ka: "ყოველდღე" },
            },
            {
              val: "every3days",
              label: {
            ...
            {
              val: "weekly",
              label: { ru: "Каждую неделю", en: "Weekly", ka: "ყოველ კვირას" },
            },
          ]
            .map((opt) => {
              const checked = reminderIntervals?.[opt.val] || false;
              return `<label style="display:flex;align-items:center;gap:12px;padding:9px 12px;background:var(--card-bg);border:1.5px solid var(--cream-border);border-radius:10px;cursor:pointer;">
              <input type="checkbox" class="reminder-interval-checkbox" data-val="${opt.val}" ${checked ? "checked" : ""}
                style="width:18px;height:18px;accent-color:var(--primary);flex-shrink:0;">
              <span style="font-size:14px;font-weight:600;color:var(--text);">${opt.label[currentLang] || opt.label.ru}</span>
            </label>`;
            })
            .join("")}
          </div>
  ...
  document
    ...
    ?.addEventListener("click", () => {
      ...
      const dt = document.getElementById("newReminderDatetime")?.value;
      if (!dt) {
        showToast(
          {
      ...
      showToast(
        "✅ " +
          {
            ru: "Напоминание запланировано",
            en: "Reminder scheduled",
            ka: "შეხსენება დაგეგმილია",
          }[currentLang],
      );
      haptic("success");
      renderSettings();
    ...
    cb.addEventListener("change", () => {
      ...
      if (activeListDiv) {
        ...
        activeListDiv.textContent = activeIntervals.length
          ...
          : {
              ru: "Нет активных интервалов",
              en: "No active intervals",
              ka: "ინტერვალები არ არის",
            }[currentLang];
      }
    });
  });
  ...
  // === ТОЧНОЕ НАПОМИНАНИЕ ===
  // (old custom reminder handlers removed — now handled by addNamedReminderBtn)
  ...
  document.getElementById("clearAllBtn").onclick = () => {
    askConfirm(
      ...
      async () => {
        ...
        // удаляем резервную копию из IndexedDB
...
function sendReminderNotification() {
  ...
  // ── Check named reminders (poller-based fallback) ──
  const list = JSON.parse(localStorage.getItem("namedReminders") || "[]");
  let changed = false;
  list.forEach((r) => {
    if (!r.fired && r.ts <= now) {
