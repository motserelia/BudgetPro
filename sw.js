...
<button class="settings-btn primary" id="importJSONBtn" style="margin-top:8px;">${t("importJSON")}</button>
      ...
      <div class="settings-card-body">
        ...
        <div id="remindersIntervalDiv" style="display:${remindersEnabled ? "block" : "none"};">
          ...
          <!-- Красивые поля даты и времени -->
          <div class="custom-datetime-row">
            <div class="custom-date-wrapper">
              <span class="datetime-icon">📅</span>
              <input type="date" id="reminderCustomDate" class="custom-date-input" value="${customReminderDate || ""}">
            </div>
            <div class="custom-time-wrapper">
              <span class="datetime-icon">⏰</span>
              <input type="time" id="reminderCustomTime" class="custom-time-input" value="${customReminderTime || ""}">
            </div>
          </div>
  ...
  if (dateInput && timeInput) {
    ...
    const commonStyle = `
      ...
      width: 100%;
    `;
    dateInput.style.cssText = commonStyle;
    timeInput.style.cssText = commonStyle;

    // Также стилизуем родительские обёртки
    document
      .querySelectorAll(".custom-date-wrapper, .custom-time-wrapper")
      .forEach((w) => {
        w.style.display = "flex";
        w.style.alignItems = "center";
        w.style.background = "var(--cream-dark)";
        w.style.border = "2px solid var(--cream-border)";
        w.style.borderRadius = "var(--radius-md)";
        w.style.padding = "0 6px";
      });
  }
...
async function showShareWelcomeScreen(pkg) {
  const ov = document.createElement("div");
  ov.style.cssText =
    "position:fixed;inset:0;background:var(--cream);z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:32px 24px;";
  ov.innerHTML = `<div style="width:80px;height:80px;border-radius:50%;background:${pkg.pcolor || "#2d6a4f"};display:flex;align-items:center;justify-content:center;font-size:42px;">${pkg.pemoji || "👤"}</div><div style="font-size:13px;font-weight:700;color:var(--text-muted);">${t("guestMode")}</div><div style="font-size:24px;font-weight:900;">${t("shareWelcome")}</div><div style="font-size:20px;font-weight:800;color:${pkg.pcolor || "var(--primary)"};">«${esc(pkg.pname || "")}»</div>${pkg.locked ? `<div style="font-size:18px;font-weight:800;color:var(--expense-color);">🔒 ${t("shareWelcomeLocked")}</div><button onclick="this.closest('[style*=fixed]').remove();init();" style="background:var(--cream-dark);border:none;border-radius:99px;padding:14px 28px;font-size:16px;font-weight:800;cursor:pointer;font-family:inherit;">${t("cancel")}</button>` : `${pkg.hasPwd ? `<div style="max-width:320px;width:100%;"><input type="password" id="shareLinkPwdIn" class="modal-input" placeholder="••••••••" style="text-align:center;font-size:20px;letter-spacing:4px;"><div id="sharePwdErrDiv" style="color:var(--expense-color);font-size:13px;text-align:center;margin-top:6px;min-height:18px;"></div></div>` : ""}<button id="joinProfileBtn" style="background:${pkg.pcolor || "#2d6a4f"};color:white;border:none;border-radius:99px;padding:18px 32px;font-size:18px;font-weight:800;cursor:pointer;font-family:inherit;">${t("shareWelcomeJoin")} →</button><button onclick="this.closest('[style*=fixed]').remove();init();" style="background:var(--cream-dark);border:1.5px solid var(--cream-border);border-radius:99px;padding:14px 28px;font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;">${t("cancel")}</button>`}`;
  document.body.appendChild(ov);
  document
    .getElementById("joinProfileBtn")
    ?.addEventListener("click", async () => {
      if (pkg.hasPwd && pkg.pwHash) {
        const entered = document.getElementById("shareLinkPwdIn")?.value || "";
        if (!entered) {
          document.getElementById("sharePwdErrDiv").textContent =
            t("enterAmount");
          return;
        }
        const h = await hashSharePwd(entered);
        if (h !== pkg.pwHash) {
          document.getElementById("sharePwdErrDiv").textContent = t("pinWrong");
          return;
        }
      }
...
function openConnectModal() {
  showToast("Connect modal - coming soon");
}
...
// Если активный профиль гостевой, а режим гостя не активен — переключаемся на владельца
const activeProf = profiles.find((p) => p.id === activeProfileId);
if (activeProf?.isShared && !sharedAccessProfile) {
  const ownerProfile =
    profiles.find((p) => p.role === "owner") ||
...
function updateSupportBadge() {
  ...
  const msgs = ownerData.supportMessages || [];
  const unread = msgs.filter((m) => !m.readByUser && !m.replied).length;
  // User sees replies
  const myMsgs = msgs.filter(
    (m) =>
      m.fromProfile === activeProfileId && m.creatorReply && !m.replyReadByUser,
  );
  const totalBadge = isCreator()
    ? msgs.filter((m) => !m.replied && !m.readByCreator).length
    : myMsgs.length;
  if (totalBadge > 0) {
...
// Patch openSupportModal to use new system
const _originalOpenSupportModal = openSupportModal;
...
function openConnectModal() {
  const L = {
    ...
    ka: {
      title: "🔗 პროფილთან დაკავშირება",
      code: "წვდომის კოდი",
      btn: "დაკავშირება",
      notFound: "პროფილი ვერ მოიძებნა",
      success: "✅ დაკავშირებულია!",
    },
  };
  const lc = L[currentLang] || L.ru;
  const html = `
    <div style="background:var(--primary-pale);padding:12px 14px;border-radius:14px;margin-bottom:14px;border-left:4px solid var(--primary);font-size:13px;line-height:1.6;">
      ${currentLang === "ru" ? "Введите код доступа, который вам прислал владелец профиля." : currentLang === "en" ? "Enter the access code shared by the profile owner." : "შეიყვანეთ წვდომის კოდი, რომელიც გამოგიგზავნა პროფილის მფლობელმა."}
    </div>
    <div class="field-group"><label class="field-label">${lc.code}</label>
    <input type="text" id="connectCodeInput" class="modal-input" placeholder="XXXXXXXX" style="font-size:18px;letter-spacing:3px;text-align:center;text-transform:uppercase;"></div>
    <div id="connectError" style="display:none;background:var(--expense-pale);color:var(--expense-color);padding:10px;border-radius:12px;margin-bottom:10px;font-weight:700;text-align:center;"></div>
    <div class="modal-actions">
  ...
  document.getElementById("connectDoBtn").addEventListener("click", () => {
    const code = document
      .getElementById("connectCodeInput")
      .value.trim()
      .toUpperCase();
    if (!code) {
      showToast(t("enterAmount"), "error");
    ...
    const foundProf = profiles.find((p) => {
      const ss = JSON.parse(
        localStorage.getItem("shareSettings_" + p.id) || "{}",
      );
      return ss.shareId === code;
    });
    if (!foundProf) {
      document.getElementById("connectError").style.display = "block";
      document.getElementById("connectError").textContent = lc.notFound;
      return;
    }
...
function injectDatetimeEnhancement() {
  ...
  style.textContent = `
    @keyframes dtPop { from{transform:scale(0.95);opacity:0} to{transform:scale(1);opacity:1} }
    ...
    @keyframes dtIconPop { 0%{transform:scale(1) rotate(0)} 40%{transform:scale(1.45) rotate(-18deg)} 100%{transform:scale(1.25) rotate(-10deg)} }

    /* Datetime wrapper — кликабельная метка */
    .custom-date-wrapper, .custom-time-wrapper {
      position: relative;
      cursor: pointer !important;
      transition: all 0.3s cubic-bezier(0.34,1.56,0.64,1) !important;
    }
    .custom-date-wrapper:hover, .custom-time-wrapper:hover {
    ...
    .custom-date-wrapper:focus-within .datetime-icon,
    .custom-time-wrapper:focus-within .datetime-icon {
      animation: dtIconPop 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards !important;
    }
    .custom-date-wrapper:hover .datetime-icon,
    .custom-time-wrapper:hover .datetime-icon {
      transform: scale(1.2) rotate(-8deg) !important;
    }
    /* Hint label under wrapper */
    .dt-open-hint {
      font-size:10px;color:var(--text-muted);text-align:center;margin-top:3px;font-weight:600;letter-spacing:0.3px;
      opacity:0;transition:opacity 0.2s ease;pointer-events:none;
    }
    .custom-date-wrapper:hover ~ .dt-open-hint,
    .custom-time-wrapper:hover ~ .dt-open-hint { opacity:1; }
    ...
    .summary-card { transition: all 0.25s cubic-bezier(0.34,1.56,0.64,1) !important; }
    ...
    .header-btn, #headerGuideBtn, #headerHelpBtn, #headerLangBtn, #themeToggle {
      transition: all 0.2s cubic-bezier(0.34,1.56,0.64,1) !important;
    }
    .header-btn:active, #headerGuideBtn:active, #headerHelpBtn:active, #headerLangBtn:active, #themeToggle:active {
  ...
  // Make icon tap open the native date/time picker
  function wireDateTime() {
        ...
        icon.title =
          {
            ...
            ka: "დააჭირეთ თარიღის ასარჩევად",
          }[currentLang] || "Pick date";
    ...
    // time
    document.querySelectorAll(".custom-time-wrapper").forEach((wrapper) => {
      const inp = wrapper.querySelector(
      ...
      if (inp && icon) {
        icon.title =
          {
            ru: "Нажмите чтобы выбрать время",
            en: "Tap to pick a time",
            ka: "დააჭირეთ დროის ასარჩევად",
          }[currentLang] || "Pick time";
        icon.addEventListener("click", (e) => {
          e.stopPropagation();
          try {
            inp.showPicker?.();
  ...
  wireDateTime();
  // Re-wire when settings tab renders (it dynamically creates datetime inputs)
  const origSetTab = window.setTab;
  if (origSetTab) {
    window.setTab = function (tab) {
      origSetTab(tab);
      setTimeout(wireDateTime, 400);
    };
  }
  // Also watch for DOM changes
  const mo = new MutationObserver(() => wireDateTime());
  mo.observe(document.body, { subtree: true, childList: true });
}
injectDatetimeEnhancement();

// ============================================================
// SUPPORT BADGE — HTML injection
// ============================================================
(function injectSupportBadgeUI() {
  // Add support button to header if not present
  const hdrActions = document.querySelector(".header-actions");
  if (!hdrActions) return;
  const existing = document.getElementById("headerSupportBtn");
  if (existing) return;
  const cs = getCreatorSettings ? getCreatorSettings() : {};
  if (cs.contactEnabled === false) return;
  const btn = document.createElement("button");
  ...
  btn.innerHTML = `💬<span id="supportBadge" style="position:absolute;top:-4px;right:-4px;background:#e53e3e;color:white;border-radius:50%;min-width:18px;height:18px;font-size:10px;font-weight:900;display:none;align-items:center;justify-content:center;border:2px solid var(--cream);padding:0 3px;"></span>`;
